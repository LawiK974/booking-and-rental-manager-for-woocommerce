<?php
	...
	function rbfw_add_cart_item_func( $cart_item_data, $rbfw_id ) {
		...
		# TARTIFLETTE
		// $cart_item_data['price'] = $duration_price_individual;
		// $cart_item_data['quantity'] = $rbfw_item_quantity;
	...
	function rbfw_show_cart_items( $item_data, $cart_item ) {
		...
		$rent_types = empty($rbfw_bikecarsd_data) ? array() : array_column($rbfw_bikecarsd_data,'price','rent_type');
		$room_desc = array_column($rbfw_resort_room_data,'rbfw_room_desc','room_type');
		$room_types = $rbfw_room_price_category == 'daynight' ? array_column($rbfw_resort_room_data,'rbfw_room_daynight_rate','room_type'): ($rbfw_room_price_category == 'daylong' ? array_column($rbfw_resort_room_data,'rbfw_room_daylong_rate','room_type') : array());
		...
		$rbfw_security_deposit_label = !empty(get_post_meta($rbfw_id, 'rbfw_security_deposit_label', true)) ? get_post_meta($rbfw_id, 'rbfw_security_deposit_label', true) : 'Security Deposit';


		// if($rbfw_rent_type == 'resort'){  // TODO
		if($rbfw_rent_type == 'resort' || $rbfw_rent_type == 'bike_car_sd' || $rbfw_rent_type == 'appointment'){
			...
			$item_data[] = array('name'    => '', 'key'     => '', 'value'   => $content, 'display' => '');
			// } elif ($rbfw_rent_type == 'bike_car_sd' || $rbfw_rent_type == 'appointment'){ // TODO
				
		} else { // equivalent to next commented line
		// } elif ($rbfw_rent_type == 'bike_car_md' || $rbfw_rent_type == 'dress' || $rbfw_rent_type == 'equipment' || $rbfw_rent_type == 'others') {
			if ( ! empty( $rbfw_pickup_point ) ):
				$item_data[] = array('name' => '', 'key' => $rbfw->get_option_trans('rbfw_text_pickup_point', 'rbfw_basic_translation_settings', __('Pickup Point','booking-and-rental-manager-for-woocommerce')), 'value' => $rbfw_pickup_point, 'display' => '');
			endif;
			if ( ! empty( $rbfw_dropoff_point ) ):
				$item_data[] = array('name' => '', 'key' => $rbfw->get_option_trans('rbfw_text_dropoff_point', 'rbfw_basic_translation_settings', __('Drop-off Point','booking-and-rental-manager-for-woocommerce')), 'value' => $rbfw_dropoff_point, 'display' => '');
			endif;
			if ( !empty($start_datetime) && !empty($start_time)):
				$item_data[] = array('name' => '', 'key' => $rbfw->get_option_trans('rbfw_text_pickup_date_time', 'rbfw_basic_translation_settings', __('Pickup Date & Time','booking-and-rental-manager-for-woocommerce')), 'value' => rbfw_get_datetime( $start_datetime, 'date-time-text' ), 'display' => '');
			else:
				$item_data[] = array('name' => '', 'key' => $rbfw->get_option_trans('rbfw_text_pickup_date', 'rbfw_basic_translation_settings', __('Pickup Date','booking-and-rental-manager-for-woocommerce')), 'value' => rbfw_get_datetime( $start_datetime, 'date-text' ), 'display' => '');
			endif;
			if ( !empty($end_datetime) && !empty($end_time)):
				$item_data[] = array('name' => '', 'key' => $rbfw->get_option_trans('rbfw_text_dropoff_date_time', 'rbfw_basic_translation_settings', __('Dropoff Date & Time','booking-and-rental-manager-for-woocommerce')), 'value' => rbfw_get_datetime( $end_datetime, 'date-time-text' ), 'display' => '');
			else:
				$item_data[] = array('name' => '', 'key' => $rbfw->get_option_trans('rbfw_text_dropoff_date', 'rbfw_basic_translation_settings', __('Dropoff Date','booking-and-rental-manager-for-woocommerce')), 'value' => rbfw_get_datetime( $end_datetime, 'date-text' ), 'display' => '');
			endif;
			if(!empty($variation_info)){
				foreach ($variation_info as $key => $value) {
					$item_data[] = array('name' => '', 'key' => $value['field_label'], 'value' => $value['field_value'], 'display' => '');
				}
			}
			if ( ! empty( $rbfw_item_quantity ) ):
				$item_data[] = array('name' => '', 'key' => $rbfw->get_option_trans('rbfw_text_quantity', 'rbfw_basic_translation_settings', __('Quantity','booking-and-rental-manager-for-woocommerce')), 'value' => $rbfw_item_quantity, 'display' => '');
			endif;
			if ( ! empty( $start_datetime ) && ! empty( $end_datetime ) ):
				$item_data[] = array('name' => '', 'key' => $rbfw->get_option_trans('rbfw_text_duration_cost', 'rbfw_basic_translation_settings', __('Duration Cost','booking-and-rental-manager-for-woocommerce')) . '('. $cart_item['total_days'].' Day'.(($cart_item['total_days']>1)?'s':'') . ')', 'value' => '('.wc_price((float)$rbfw_duration_price_individual) .' x '.$rbfw_item_quantity.')'. ' = '.wc_price((float)$rbfw_duration_price_individual * $rbfw_item_quantity), 'display' => '');
			endif;
			if ( ! empty( $rbfw_service_infos ) ){
				foreach ($rbfw_service_infos as $key => $value){
					if(count($value)){
						foreach ($value as $key1=>$item){
							if ($item['service_price_type']=='day_wise') {
								$content = '(' . wp_kses(wc_price($item['price']),rbfw_allowed_html()) .  'x' .  esc_html($item['quantity'])  .  'x'  . esc_html($total_days)  . '=' . wp_kses(wc_price($item['price']*(int)$item['quantity']*$total_days),rbfw_allowed_html()) . ')';
							} else {
								$content = '(' . wp_kses(wc_price($item['price']),rbfw_allowed_html()) .  'x' .  esc_html($item['quantity'])  . '=' . wp_kses(wc_price($item['price']*$item['quantity']),rbfw_allowed_html()) . ')';
							}
							$item_data[] = array('name' => '', 'key' => $item['name'], 'value' => $content, 'display' => '');
						}
					}
				}
			}
			...
			if ( ! empty( $rbfw_service_info ) ){
				foreach ($rbfw_service_info as $key => $value){
                    ...
                    if(array_key_exists($service_name, $extra_services)){ // if service name exist in array
						...
						$item_data[] = array('name' => '', 'key' => $service_name, 'value' => wc_price($service_price) . 'x' . $service_qty . '=' . wc_price($total_service_price), 'display' => '');
					}
				}
			}

			if (  $discount_amount ):
				$item_data[] = array('name' => '', 'key' => $rbfw->get_option_trans('rbfw_text_discount', 'rbfw_basic_translation_settings', __('Discount','booking-and-rental-manager-for-woocommerce')), 'value' => wc_price($discount_amount), 'display' => '');
			endif;
			if ( ! empty( $security_deposit_amount ) ):
				$item_data[] = array('name' => '', 'key' => $rbfw_security_deposit_label, 'value' => wc_price($security_deposit_amount), 'display' => '');
			endif;
		}
